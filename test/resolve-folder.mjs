...
const PORT = "8798";
const importFile = join(tmpdir(), `cbm-resolve-folder-${process.pid}.json`);
...
send(7, "add_bookmark", { title: "X", url: "https://example.test", parent_id: "" });
...
try { unlinkSync(importFile); } catch { /* gone */ }
...
const emptyId = (id) => {
  const text = textOf(responses.get(id));
  return /empty parent id/.test(text) &&
    !/empty path|provide to_path or to_parent_id|Bookmarks bar|Other bookmarks|not connected/.test(text);
};

check("add_bookmark — parent_id \"\" throws empty parent id (not default root)",
  emptyId(7), textOf(responses.get(7)).slice(0, 160));
check("create_folder — parent_id \"\" throws empty parent id (not default root)",
  emptyId(8), textOf(responses.get(8)).slice(0, 160));
check("move_bookmark — to_parent_id \"\" throws empty parent id (not provide-to_path)",
